...
void DrawTutotial(Vector2& Mouse)
{
	...
	DrawText("How to Play", GetScreenWidth() / 3, GetScreenHeight() / 9 - 50, 60, GOLD);
	DrawText("Press W to make the penguin fly" , GetScreenWidth() / 2 - 150 , GetScreenHeight() / 5 + 60, 20, GOLD);
	DrawText("Avoid the pipes and try to stay alive!", GetScreenWidth() / 2 - 170, GetScreenHeight() / 5 + 90, 20, GOLD);
	DrawText("Press The Back menu botton to go back to the main menu.", GetScreenWidth() / 2 - 270, GetScreenHeight() / 5 + 120, 20, GOLD);
